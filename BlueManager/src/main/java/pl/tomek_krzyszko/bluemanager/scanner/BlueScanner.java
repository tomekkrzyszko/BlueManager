...
import pl.tomek_krzyszko.bluemanager.dagger.components.ScannerComponent;
...
public class BlueScanner extends Service {
    ...
    private BlueScannerSettings settings;
    ...
    void onFailure(int errorCode) {
        ...
        for (final BlueDeviceScanListener wooletListener : wooletListenersCopy) {
            ...
            wooletListener.onDeviceScanError(errorCode);
        }
    ...
    public synchronized void start() {
        if (runTask == null) {
            ...
            runTask = new BlueScannerTask(this);
            new Thread(runTask).start();
        } else {
            Timber.i("BlueScanner already started.");
        }
    }
     ...
     */
    private BlueScannerTask runTask;

    public Set<BluetoothDevice> getBondedDevicesSet(){
        if(runTask!=null){
            return runTask.getBondedDevicesSet();
        }else{
